...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($checkVote->num_rows > 0) {
        ...
        //update tokens
        $sql = "UPDATE tokens SET amount = amount + 1 WHERE user_id = $userId";
        $conn->query($sql);

        // Insert transaction history
        $st = $conn->prepare("INSERT INTO token_transactions (user_id, type, amount, description, created_at) VALUES (?, 'earn', 1, 'Polls and Surveys Activity Reward', NOW())");
