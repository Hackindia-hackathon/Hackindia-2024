...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        $usernameError = "Username or email already exists. Please choose a different username or email.";
    } else {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            $sql = "INSERT INTO tokens (user_id,amount) VALUES ('$lastUserId','100')";
            $conn->query($sql);

             // Insert transaction history
            $stmt = $conn->prepare("INSERT INTO token_transactions (user_id, type, amount, description, created_at) VALUES (?, 'earn', 100, 'Welcome Bonus', NOW())");
            $stmt->bind_param("s", $lastUserId);
            $stmt->execute();
            $stmt->close();
