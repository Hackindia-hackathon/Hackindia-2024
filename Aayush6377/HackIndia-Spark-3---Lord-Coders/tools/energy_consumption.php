    ...
    <main>
        <div class="energy-tracker-container">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                // Average usage (kWh per year)
                $average_lightbulbs = 0.06 * 365 * 1; // Assuming 1 lightbulb
                $average_tv = 0.1 * 365 * 4; // Assuming 4 hours per day
                $average_fridge = 1.2 * 365; // Fridge usage
                $average_ac = 2.0 * 365 * 1; // Assuming 1 AC running for 1 hour per day
                $average_washing_machine = 2 * 52; // Weekly usage

                $average_total_energy = $average_lightbulbs + $average_tv + $average_fridge + $average_ac + $average_washing_machine;

                // Recommendations
                $recommendation = "";
                if ($total_energy < $average_total_energy) {
                    $recommendation = "Your energy consumption is below average. Great job on being energy efficient!";
                } elseif ($total_energy == $average_total_energy) {
                    $recommendation = "Your energy consumption is at average levels. Consider reducing usage to be more energy efficient.";
                } else {
                    $recommendation = "Your energy consumption is above average. You might want to check for ways to reduce your energy usage.";
                }
