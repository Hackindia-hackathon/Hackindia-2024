    ...
    <main>
        <div class="tracker-container">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $sql = "SELECT COUNT(*) AS total_entries FROM fitness_tracker WHERE user_id = '$user_id'";
                $result = $conn->query($sql);
                $row = $result->fetch_assoc();
                $total_entries = $row['total_entries'];

                // Initialize base token and calculate increment
                $base_token = 4;
                $increment = intval($total_entries / 10) * 2;

                // Ensure the token doesn't exceed 30
                $tokens = min($base_token + $increment, 30);

                

                $sql = "INSERT INTO fitness_tracker (user_id, date, exercise, duration, calories,tokens_awarded) VALUES ('$user_id', '$date', '$exercise', '$duration', '$calories','$tokens')";

                if ($conn->query($sql) === TRUE) {
                    echo "<p>Entry added successfully!</p>";
                    echo "<p>You have earned $tokens tokens.</p>";

                    $sql = "INSERT INTO token_transactions (user_id, type, amount, description) VALUES ('$user_id', 'earn', '$tokens', 'Fitness Tracker Activity Reward')";
                    $conn->query($sql);

                    $sql = "UPDATE tokens SET amount = amount + $tokens WHERE user_id = $user_id";
                    $conn->query($sql);
                } else {
            ...
            <table>
                ...
                <?php
                $user_id = $_SESSION['user_id']; // Get the logged-in user's ID
                $sql = "SELECT date, exercise, duration, calories FROM fitness_tracker WHERE user_id = '$user_id' ORDER BY date DESC";
                $result = $conn->query($sql);
